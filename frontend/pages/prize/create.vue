    ...
    <div class="container mx-auto mt-10">
      <el-card>
        <form @submit.prevent="submitForm">
          ...
          <div class="mt-4">
            <Label for="prize_code">Naujas kodas</Label>
            ...
            <input
              type="button"
              class="mt-4 mr-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded "
              value="Pridėti"
              @click="addPrizeCode"
            />
          </div>
          ...
          <!-- prize codes -->
          <div class="mt-4" 
          v-if="data.prize_codes.length > 0"
          >
            ...
            <div class="flex flex-col">
              <div v-for="(prizeCode, index) in data.prize_codes" :key="index">
                <div class="flex flex-row justify-between">
                  <div
                    class="flex flex-col w-1/4 mr-1 mt-4 mb-4 border-2 border-gray-200 rounded-lg p-4"
                  >
                    <div class="justify-between">
                      <!--div for label and button -->
                        <!--input -->
                        <input
                          type="button"
                          class="h-10 w-full
                           bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mb-4 "
                          value="Ištrinti"
                          @click="removePrizeCode(index)"
                        />
                      <Input
                        id="prize_code"
                        class="block mt-1 w-full"
                        v-model="prizeCode.code"
                        :errors="errors.prize_code"
                      />
                    </div>
                  </div>
