...
async function loadLitters() {
  const response: any = await $fetch("/api/litter", {method: 'GET', query: {page: 1, perPage: 100}});
  if (response.status) {
    //where litter is not cleadned 
    LitterList.value = response.data.data.filter((litter: Litter) => litter.is_cleaned === 0);
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-12 cursor-default">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          <h2 class="lg:flex font-semibold text-xl text-gray-800 leading-tight m-2 cursor-default select-none" >
            ...
            <div class="flex justify-around mt-4 lg:mt-0 flex-wrap align-items-center gap-y-4">
            <div class=" ml-10 inline-flex px-3 py-0.5 rounded-full text-sm font-medium leading-5 bg-green-100 text-green-800
            " v-if="userStore.isLoggedIn">
              Jūsų turimi taškai: {{userStore.getPoints}}
            </div>
